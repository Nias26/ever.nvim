require("config.lazy")
require("config.options")
require("config.keymaps")
require("config.tabline").setup()
require("config.autocmds")

-- TODO: Add gitsigns.nvim
-- TODO: Create Terminal command for vsplit term, split term, floating term, new tab term or whole buffer term
-- TODO: Fix Conform failing on js files
-- TODO: Fix 'K' keymap getting pressed multiple times
-- TODO: Fix 'Q' closing all buffers when inside a tab. Instead switch to other buffer and if none, open a new buffer. Never close tab until said with ':tabclose'
